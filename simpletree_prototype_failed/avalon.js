...
(function (DOC) {
     ...
     *                           配置系统                                 *
     **********************************************************************/
    function kernel(settings) {//即requirejs的config方法
    ...
    function addAssign(vars, scope, name, duplex) {
        ...
        for (var i = vars.length, prop; prop = vars[--i];) {
            if ((scope.hasOwnProperty && scope.hasOwnProperty(prop))
                || prop in scope) { //IE6下节点没有hasOwnProperty
    ...
    var getBindingCallback = function (elem, name, vmodels) {
        ...
        if (callback) {
            ...
            //add at 2014.10.7 Function Patch data-include-loaded/rendered等回调函数支持在el中寻找
            var params = callback.split('.')
            if (params.length == 2) {
                for (var i = 0; i < vmodels.length - 1; i++) {
                    vm = vmodels[i];
                    if (vm.hasOwnProperty(params[0]) && vm[params[0]].hasOwnProperty(params[1])
                        && typeof vm[params[0]][params[1]] === 'function') {
                        return vm[params[0]][params[1]]
                    }
                }
            }
        }
...
//patch 2256 : || prop in scope 能判断在原型中的类型